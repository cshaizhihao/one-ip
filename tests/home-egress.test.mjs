...
function render(primary, split, client = {}, capture = () => {}) {
  ...
  const require = (name) => {
    ...
    if (name === "@/hooks/use-available-tools")
      return { useAvailableTools: () => [] };
    if (name === "@/layout/routes")
      return { toolGroups: { network: [], ai: [] } };
...
test("matching domestic and external IPv4 keeps both probe lanes", () => {
  const result = render([{ data: ipv4 }, { data: ipv4 }, failed], []);
  assert.ok(result.includes("IPv4 · 国内探测"));
  assert.ok(result.includes("IPv4 · 外部探测"));
  assert.ok(result.includes("国内与外部探测使用同一出口"));
});
